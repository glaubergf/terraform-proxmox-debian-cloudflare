...
resource "local_file" "cloud_init" {
  ...
  filename = "${path.module}/configs/cloudflare_cloud_init.cfg"
}

resource "local_file" "network_config" {
  content  = data.template_file.network_config.rendered
  filename = "${path.module}/configs/cloudflare_network_config.cfg"
}
...
resource "null_resource" "cloud_init" {
  ...
  provisioner "file" {
    ...
    destination = "/var/lib/vz/snippets/cloudflare_cloud_init.yml"
  }

  provisioner "file" {
    source      = local_file.network_config.filename
    destination = "/var/lib/vz/snippets/cloudflare_network_config.yml"
  }
}
...
resource "proxmox_vm_qemu" "debian_nocloud" {
  ...
  cicustom = "user=${var.storage_proxmox}:snippets/cloudflare_cloud_init.yml,network=${var.storage_proxmox}:snippets/cloudflare_network_config.yml"
...
resource "null_resource" "debian_nocloud" {
  ...
  provisioner "file" {
    source      = var.motd_cloudflare_path
    destination = "/tmp/motd_cloudflare"
  }
  ...
  provisioner "file" {
    source      = var.env_cloudflared_token_path
    destination = "/tmp/.env"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # Rodar o container do Cloudflare Tunnel sempre, com reinício automático
      #format("docker run -d --restart unless-stopped cloudflare/cloudflared:latest tunnel --no-autoupdate run --token %s", var.cloudflared_token),

      # Executar script motd e o docker compose.
      "sudo chmod +x /tmp/config_motd.sh",
      "sudo bash /tmp/config_motd.sh",
      "sudo mkdir -p /root/docker_cloudflare",
      "sudo mv /tmp/docker-compose.yml /root/docker_cloudflare/docker-compose.yml",
      "sudo mv /tmp/.env /root/docker_cloudflare/.env",
      "sudo docker compose -f /root/docker_cloudflare/docker-compose.yml up -d"
    ]
  }
}
